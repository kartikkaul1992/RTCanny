        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
int n_thread1 = 4; //Edited by Jiang Wan for multi-thread
omp_set_dynamic(0); //Edited by Jiang Wan for multi-thread
omp_set_num_threads(n_thread1); //Edited by Jiang Wan for multi-thread
#pragma omp parallel private(c, cc, dot, sum) //Edited by Jiang Wan for multi-thread
{
if(VERBOSE) printf(" Bluring the image in the Xdirection.\n");
#pragma omp for
for(r=0;r<rows;r++){
for(c=0;c<cols;c++){
dot = 0.0;
...
tempim[r*cols+c] = dot/sum;
   ...
   ****************************************************************************/
int n_thread2 = 4; //Edited by Jiang Wan for multi-thread
omp_set_dynamic(0); //Edited by Jiang Wan for multi-thread
omp_set_num_threads(n_thread2); //Edited by Jiang Wan for multi-thread
#pragma omp1 parallel private(r, rr, dot, sum) //Edited by Jiang Wan for multi-thread
{
...
(*smoothedim)[r*cols+c] = (short int)(dot*BOOSTBLURFACTOR/sum
+ 0.5);
}
}
}
