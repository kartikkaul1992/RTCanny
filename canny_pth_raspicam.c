...
// for blurY threading, added by kartikk
...
**********************************************************************
******/
struct thread_args_y *args = (thread_args_y*) arguments;
...
**********************************************************************
******/
struct thread_args_x *args = (thread_args_x*) arguments;
unsigned char *image = args->image;
int rows = args->rows;
int cols = args->cols;
...
for(i=0;i<N_T;i++){
    ...
    iret_y[i] = pthread_create(&thread_y[i], NULL, &blur_y, &args_y[i]);
}
for(i=0; i<N_T; i++)
{
